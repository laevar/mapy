...
\begin{columns}[c]
 \column{0.4\textwidth}
{\it Beispiel:} Durch \\
\alert{ \mcode{plot(x,y,'r*--')}} wird die Linie
...
\alert{ \mcode{plot(x1,y1,string1,x2,y2,string2,...)}}
\item Logaritmische Skalierung in $x$- bzw in $y$-Richtung: \alert{ \mcode{semilogx(x1,y1)}} bzw. \alert{
  \mcode{semilogy(x1,y1)}}.
\item Logarithmische Skalierung beider Achsen: \alert{
  \mcode{loglog(x1,y1)}}   
\item Ist $X$ ein Vektor mit komplexen Einträgen, so ergibt \alert{
  \mcode{plot(X)}} \mcode{plot(real(X),imag(X))}.
\end{itemize}
\end{frame}
% 
% Slide
...
\begin{itemize}
\item Durch \mcode{strcat} werden Strings verbunden, z.B. \mcode{strcat('Hello',' world')}. 
%\begin{lstlisting}
%>> strcat('Hello',' world')
%ans =
%Hello world
%\end{lstlisting}
\item \mcode{num2str(x,n)} konvertiert $x$ in einen String mit $n$
  signifikanten Stellen. (Default: $n=4$)
\item \mcode{int2str(x)} rundet $x$ und konvertiert es in einen String.
\item \mcode{strcmp(s,t)} vergleicht die Strings $s$ und $t$. 
\item Durch \mcode{help strfun} erhält man eine Liste aller Befehle im
...
\item \mcode{title('$f(x) = \\frac\{1\}\{x^2+a\}$','interpreter','latex')} $\Rightarrow$ $f(x) = \frac{1}{x^2+a}$
\end{itemize}
...
\begin{itemize}
\item Öffnen eines (weiteren) Grafikfensters: \mcode{figure}.  Eine
  Grafik wird immer im aktuellen Fenster erzeugt. Ist noch kein
  Fenster geöffnet, so wird ein Fenster erzeugt.
\item Durch den Befehl \mcode{hold on} werden bestehende Grafiken im
  aktuellen Fenster erhalten. Neue Grafiken werden den bestehenden
  hinzugefügt. 
\item \mcode{hold off} (default) überschreibt Grafiken im
  aktuellen Fenster
\item Schliessen: \mcode{close}, \mcode{close all}
\end{itemize}
\end{frame}
...
\end{minipage}\\
\begin{itemize}
\item Balkendiagramm: \alert{ \mcode{bar(y)}}
\item Histogramm: \alert{ \mcode{hist(y,5)}}
\item einfacher Plot: \alert{ \mcode{area(n,[y',2*y'])}}
\item Tortengrafik:  \alert{ \mcode{pie3([ 1 2 3 4])}}
\end{itemize}
\end{frame}
% 
% Slide
...
\begin{itemize}
\item \mcode{contour(X,Y,Z,n)} zeichnet f\"ur $n\in \mathbb{N}$
  $n$-Konturlinien. Ist $n$ ein Vektor, werden Konturlinien zu den Werten in
  dem Vektor $n$ geplottet.
\item \mcode{contourf} funktioniert wie \mcode{contour} nur das die Flächen
  zwischen den Konturlinien ausgefüllt werden.
\item \mcode{label(C,h)} beschriftet die Konturlinien, deren Werte in $C$
  gespeichert sind und die zum Grafik-Handle $h$ gehören.
\item \mcode{contour3} zeichnet jede Konturlinie auf einer anderen H\"ohe.
\end{itemize}
\end{frame}
%
% Slide
...
\begin{itemize}
\item Mit \mcode{F(j)=getframe} wird die aktuelle Grafik in das Array
  $F$ gespeichert.
\item Nach dem alle Bilder zu $F$ hinzugefügt worden sind, kann man
  die Sequenz der Bilder $F$ darstellen durch \mcode{movie(F,n,fps)},
  wobei $n$ die Anzahl der Wiederholungen angibt und $fps$ der
  gezeigten Frames pro Sekunde entspricht (Default: $n=1$, $fps=12$). 

\item Speichern des Movies in AVI Format: \mcode{movie2avi(F,Dateiname)}
\end{itemize}
\end{frame}
...
In MATLAB: \alert{ \mcode{p=polyfit(x,y,n)}} \\[0.5cm]

Ruft man \alert{ \mcode{p=polyfit(x,y,m)}} mit $m<n$ auf, so sucht MATLAB
