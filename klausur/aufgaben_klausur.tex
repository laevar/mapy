...
\emph{Tipp:} \mcode{ischar} überprüft, ob eine Variable ein Character-Array ist.
\end{aufg}
...
\emph{Hinweis:} \mcode{csvread} darf nicht benutzt werden, es brauchen keine Überprüfungen gemacht werden.\\
\emph{Tipp:} \mcode{fscanf} liest ohne zusätzlichen Parameter solange ein, bis der Format-String nicht mehr passt.
\end{aufg}
...
\[
\int_0^1 \phi(x)e^{-\abs{x-y}}\sin(\abs{x-y})^2 dx
\]
für $y \in [0,1]$ mit der Dichte $\phi(x)$. Für gegebenes $x \in [0,1]$
konstruieren sie die Matrix $A$ so, dass die Gleichung 
\[
 A \phi = f
\]
gilt, wobei $f$ gegeben ist durch $f(x)=\sin(x),  x \in [0,1]$. Weiterhin kann der Einfachheit halber angenommen werden, 
dass $x_i = y_i$ für alle Komponenten $i$ der Koordinatenvektoren.
Lösen sie das Gleichungssystem nach $\phi$.
